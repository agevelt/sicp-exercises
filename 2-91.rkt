...
(define (install-polynomial-package)
    ...
    (if (same-variable? (variable p1) (variable p2))
        (let ((result (div-terms (term-list p1) (term-list p2))))
          (let ((quotient (car result))
            ...
            ; The exercise text in 2.91 says to return a list of the quotient and remainder polys.
            ; Ideally we would have a way to represent the remainder in a way that lets us further
            ; compute on it, but in this case we're settling for a way to display the result nicely.
            (list (make-poly (variable p1) quotient)
                  `(/ ,(make-poly (variable p1) remainder) ,p2))))
  ...
  (define (div-terms L1 L2)
    (if (empty-termlist? L1)
        ...
        (let ((t1 (first-term L1))
              (t2 (first-term L2)))
          (if (> (contents (order t2)) (contents (order t1)))
              (list (the-empty-termlist (type-tag L1)) L1)
              (let ((new-c (div (coeff t1) (coeff t2)))
                    (new-o (sub (order t1) (order t2))))
                (let ((rem (mul-term-by-all-terms (make-term new-o new-c) L2)))
                  (let ((rest-of-result (div-terms (sub-terms L1 rem) L2)))
  ...
  (put 'div '(polynomial polynomial)
       (lambda (p1 p2) (attach-tag 'poly-with-remainder (div-poly p1 p2))))
  (put '=zero? '(polynomial)
...
;   (rational 5.0 . 2.0)
;   (integer . 0)
;   (complex rectangular (integer . -6) integer . 2))
(div (make-sparse-polynomial 'x (list 1 2 -7))
     (make-sparse-polynomial 'x (list 1 -2)))
; '(poly-with-remainder
;   (x sparse-termlist (term (integer . 1) (integer . 1)) (term (integer . 0) (integer . 4)))
;   (/ (x sparse-termlist (term (integer . 0) (integer . 1))) (x sparse-termlist (term 1 1) (term 0 -2))))

(div (make-dense-polynomial 'x (list 1 2 -7))
     (make-dense-polynomial 'x (list 1 -2)))
; '(poly-with-remainder
;   (x dense-termlist (integer . 1) (integer . 4))
;   (/ (x dense-termlist (integer . 1)) (x dense-termlist 1 -2)))
